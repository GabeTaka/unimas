...
class UploadHandler
{
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null, $file_name = null) {
        $this->response = array();
        $this->options = array(
            //'script_url' => $this->get_full_url().'/'.$this->basename($this->get_server_var('SCRIPT_NAME')).'/file/do_upload',
            'file_name' => $file_name,
            'script_url' => $this->get_full_url().'/'.$this->basename($this->get_server_var('SCRIPT_NAME')).'/file/delete_file/',
    ...
    protected function set_additional_file_properties($file) {
        //$file->deleteUrl = base_url()."file/delete_file/".rawurlencode($file->name);
//        $file->deleteUrl = $this->options['script_url']
//            .$this->get_query_separator($this->options['script_url'])
//            .$this->get_singular_param_name()
//            .'='.rawurlencode($file->name);
        $file->deleteUrl = $this->options['script_url']; //new url (phangsw)
        //$file->deleteType = $this->options['delete_type'];
        $file->deleteType = rawurlencode($file->name); //overried (phangsw)
    ...
    protected function generate_unique_filename($filename = "")
    {
        ...
        return md5($filename.date('Y-m-d H:i:s:u')) . $extension;
    }
...
//            $index, $content_range);
    ...
    protected function get_file_name_param() {
        ...
        if($this->options['file_name']){
            return $this->basename(stripslashes($this->options['file_name']));
        }
